...
// roles: 'admin' = all, 'viewer' = dashboard + eerr only
const navItems = [
  {
    to: '/dashboard',
    label: 'Dashboard',
    ...
    <aside className="w-64 flex-shrink-0 bg-slate-800 flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-slate-700">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center flex-shrink-0 shadow-lg shadow-emerald-500/30">
            <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
            </svg>
          </div>
          <div>
            <h1 className="text-white font-bold text-base leading-tight">Resultados</h1>
            <h1 className="text-emerald-400 font-bold text-base leading-tight">Bacanes</h1>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-3 border-t border-slate-700">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-slate-700/50">
          <div className="w-8 h-8 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-semibold text-sm flex-shrink-0">
            {user?.nombre?.charAt(0).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-white text-xs font-medium truncate">{user?.nombre}</p>
            <p className="text-slate-500 text-xs truncate">{user?.role === 'admin' ? 'Administrador' : 'Solo lectura'}</p>
          </div>
          <button
